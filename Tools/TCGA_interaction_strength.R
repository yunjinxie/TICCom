...
TCGA_interaction_strength <- function(IDPath=NULL,expPath=NULL,compare_expPath = NULL,gene_listPath=NULL,cancer=NULL,referencePath=NULL,show_level="gene",source = "TCGA",outpath){
    ...
    if(is.null(stopMessage)){
      ...
      if(sum(cancer %in% "empty")==0){
        ...
        if(sum(show_level %in% "patient")==1){
          ...
          score_result <- lapply(1:length(cancer),function(x) Interaction_strength_gene(exp_rank = exp_rank[[x]],pairs_immune = pairs2,cancer = cancer[x],r_score = random_strength[[x]]))
          names(score_result) <- cancer
          is_null <- unlist(lapply(score_result,function(x) is.null(x[[1]])))
          cancer2 <- cancer[!is_null]
        ...
        if(nrow(gene2)==0){
          ...
          if(sum(show_level %in% "patient")==1){
            ...
            score_result <- Interaction_strength_gene(exp_rank = exp_rank3,pairs_immune = pairs2)
            plot_data <- score_result[[1]]
...
######################gene
Interaction_strength_gene <- function(exp_rank,pairs_immune,cancer = NULL,r_score = NULL){
  pairs_immune$pairs2 <- paste(pairs_immune$Gene1Name,pairs_immune$Gene2Name,sep='_')
