##################
ICCom <- function(expPath = "empty",compare_expPath = "empty",p_value = 0.1,referencePath,databaseFile = "iTALK_LR_database",
      ...
      if(show_level == "gene"){
        ...
        if(is.null(plot_data)){
          ...
          if(nrow(plot_data_sig)==0){
            ...
            pp <- paste(plot_data_df$ligand,plot_data_df$receptor,sep='_')
            plot_data_df$curated <- database[match(pp,database$pair),,drop=F]$Evidence
