# find significant LR pairs with iTALK
# if chose one of the comm_type,all is FALSE, otherwise all is TRUE
# gene names can chose from symbol,ensembl,entrez
# database can 
# top_genes (scale 1 to 100)  10,20,30,40,50
# organism (human,mouse)
# databaseFile (iTALK_LR_database,icellnet_LR_database,RNAMagnet_mouse_LR_database)
iTALK_method_top_genes <- function(dataFile,metaFile,referencePath,gene_names = "symbol",top_genes = 50,organism = "human",stats = "mean",outPath,
                                   databaseFile = "icellnet_LR_database"){
  suppressMessages({
    library(iTALK)
    library(tidyverse)
    library(jsonlite)
  ...
  tryCatch({
        ...
        if(length(inter_ID)==0){
          ...
          if(sum(dim(res_cat)[1])==0){
            ...
            #######======================================output table
              ...
              cat(jsoncars, file = paste(outPath,'iTALK_method_top_genes_r_re.txt',sep='/'), fill = FALSE, labels = NULL, append = FALSE)
            #########plot barplot
              ...
              #write.table(t(c(xAxis_s,yAxis)),paste(outPath,"iTALK_method_top_genes_r_re_barplot.txt",sep='/'),sep='\t',quote = F,row.names = F,col.names = F)
            ########plot riverplot
              res_cat_sort <- res_cat[order(res_cat[,"score"],decreasing = T),]
              ...
              cat(river_jsoncars, file = paste(outPath,'iTALK_method_top_genes_r_re_riverplot.json',sep='/'), fill = FALSE, labels = NULL, append = FALSE)
            ######################################################plot circles
              cell_pairs <- paste(res_cat[,"cell_from"],res_cat[,"cell_to"],sep='_')
              res_cat3 <- cbind(res_cat2,cell_pairs)
              
              cell_count2 <- aggregate(res_cat3[,"gene_pairs"],list(res_cat3[,"cell_pairs"]),function(x) length(unique(x)))
              cell_count_sort <- cell_count2[order(cell_count2[,2],decreasing = T),]
              
              ###################network
              res_cat_plot <- res_cat3[res_cat3[,"cell_pairs"]==cell_count_sort[1,1],,drop=F]
              ...
              jsoncars <- toJSON(output, pretty=TRUE)
              cat(jsoncars, file = paste(outPath,"iTALK_method_top_genes_r_re_circle.json",sep='/'), fill = FALSE, labels = NULL, append = FALSE)
          }
        }
      }
    }
    ...
    if(is.null(stopMessage)){
      ...
      download_table="iTALK_method_top_genes_r_re.txt"
      Res_barplot_x=xAxis
      Res_barplot_value=yAxis
      Res_riverplot_file="iTALK_method_top_genes_r_re_riverplot.json"
      Res_circle_file="iTALK_method_top_genes_r_re_circle.json"
      resut_merge=paste0("{",
      ...
      Task_ID <- as.matrix(read.table(paste0(referencePath,"/Job_ID.txt"),header = F,sep = "\t",check.names = F,fill = T))
      Task_ID_new <- Task_ID
      Task_ID_new[which(Task_ID_new[,1]==ID_new),2] <- "iTALK_method_top_genes+success"
      write.table(Task_ID_new,paste0(referencePath,"/Job_ID.txt"),quote = F,sep = "\t",row.names = F,col.names = F)
